...
def main_func():
    ...
    with open("./testinput.txt", "r") as file:
        ...
        # Modyfing lines for part 2
        for line in data:
            line_init = line["draw"]
            nb_init = line["numbers"].copy()
            for i in range(4):
                line["draw"] = line["draw"] + "?" + line_init
                for j in range(len(nb_init)):
                    line["numbers"].append(nb_init[j])
            line["draw"] = list(line["draw"])
